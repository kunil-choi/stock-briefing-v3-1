...
- FIX-CONTEXT-5     : 종목 "남성"(004270)이 "남성 선수는..." 같은 일반
                     명사(男性) 용법으로 관심종목 후보 1위까지 오르던 사고
                     수정. _ALWAYS_REQUIRE_FINANCE_CONTEXT에 추가해 예스24와
                     동일하게 근처에 실질적 투자 맥락이 있을 때만 인정.
"""
...
#
# FIX-CONTEXT-5: 종목 "남성"(004270)은 "남성 선수는 1만 2435명"처럼 일반
# 명사("male/man")로 훨씬 더 자주 쓰이는 이름이라, 광고·협찬 트리거 단어가
# 전혀 없는 순수 사회면/스포츠 뉴스에서도 문법적으로 완전히 유효한 단어
# 경계로 매칭돼 관심종목 후보 1위까지 오른 사고가 있었다(2026-08-18). "태양"
# (FIX-CONTEXT-1)처럼 특정 트리거 단어 몇 개로 막을 수 있는 동음이의어가
# 아니라 일상 대화 전반에 두루 쓰이는 단어이므로, 예스24와 동일하게 "이
# 이름 근처에 실질적 투자 맥락이 있을 때만 언급으로 인정"하는 방식으로
# 전환한다.
_ALWAYS_REQUIRE_FINANCE_CONTEXT = {"예스24", "남성"}
